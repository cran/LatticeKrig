...
"predict.se.LKrig" <- function(object, xnew=NULL, Znew = NULL, drop.Z = FALSE, verbose = FALSE, ...) {
    if( is.null(object$Mc)){
      stop("need to include the sparse cholesky decompostion in LKrig object \r
...
# (ind.drift has the indices of just the spatial drift part --- e.g. the linear polynomial)
    if (is.null(Znew) & object$nZ > 0) {
      ...
      T.matrix<- cbind( fields.mkpoly(object$x, m = 2) , object$Z)
#     
    if (drop.Z | object$nZ == 0){
      ...
      t0 <- t(cbind(fields.mkpoly(xnew, m = 2), Znew))
      Omega<- object$Omega
    } 
#    
...
# colSums used to find multiple quadratic forms  e.g.  diag(t(x) %*%A%*%x) == colSums( x* (A%*%(x)))
    temp1<-  rho * (colSums(t0*(Omega %*% t0)) -
                    colSums(k0*hold$c.mKrig) - 2*colSums(t0*d.coef))
